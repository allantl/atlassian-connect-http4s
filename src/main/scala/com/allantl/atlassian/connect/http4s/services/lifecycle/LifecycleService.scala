...
class LifecycleService[F[_]: Monad: Logger](hostRepo: AtlassianHostRepositoryAlgebra[F]) {
        ...
        case Some(existing) =>
          val logInstallEvent =
            if (existing.baseUrl != installEvent.baseUrl) {
              Logger[F].warn(s"Updating baseUrl from ${existing.baseUrl} to ${installEvent.baseUrl} for host $existing")
            } else {
              Logger[F].info(s"Reinstalling addon with baseUrl ${existing.baseUrl} for host $existing")
            }
  ...
  def uninstall(uninstallEvent: UninstallEvent): EitherT[F, HostError, Unit] =
    for {
      host <- EitherT(hostRepo.findByClientKey(uninstallEvent.clientKey).map(_.toRight(HostNotFound: HostError)))
